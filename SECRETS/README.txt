...
│   ├── ca-admin.p12 : stored on ca-admin personal machine
│   └── gen_pkcs12.sh
├── ca-server
...
│   ├── ca-server-ssh
│   └── ca-server-ssh.pub
├── firewall-ssh : SSH credentials to backup firewall config to backup server
│   ├── firewall-ssh
│   └── firewall-ssh.pub
...
│   ├── webserver-https.crt
│   └── webserver-https.key
└── webserver-intranet : certificate and key for TLS client authentication when webserver connects to MySQL db.
    ├── webserver-mysql.crt
    └── webserver-mysql.key
